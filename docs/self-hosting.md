...
every token's scrypt hash, id and operator-chosen name — so the server
creates and keeps the file at `0600` from the moment it exists, with no write
ever publishing it at a wider mode. The image creates `/data` owner-only,
which a freshly created volume inherits; a volume from an older release that
kept a looser mode is tightened to `0700` on startup too, on a best-effort
basis. Keep those permissions on any copy you make: treat a backup of it the
