...
| `STREAMWALL_AUTH_RATE_LIMIT_MAX`     | `10`       | Stricter max per IP per window for every request that verifies a token against its scrypt hash: `/invite/:id`, the uplink handshake, and a first request on a session the server has not seen recently. Requests on an already-verified session, and the static client, stay on the global budget.                                 |
