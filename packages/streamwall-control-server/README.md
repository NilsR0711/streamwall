...
| `STREAMWALL_AUTH_RATE_LIMIT_MAX`     | `10`       | Stricter max, per IP per window and counted separately for each route, on requests that verify a token against its scrypt hash: `/invite/:id`, the uplink handshake, `/admin/status` and the `/client/ws` upgrade. Requests on a credential the server verified moments ago, and requests carrying none at all, are not charged against it — they stay on the global budget. |
